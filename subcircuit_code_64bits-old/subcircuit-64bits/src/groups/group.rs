use super::super::scalar::{Scalar, ScalarBytes, ScalarBytesFromScalar};
use super::group_errors::GroupError;
use core::borrow::Borrow;
use core::ops::{Mul, MulAssign};

pub type GroupElement = curve25519_dalek::ristretto::RistrettoPoint; // pub struct RistrettoPoint(pub(crate) EdwardsPoint)
pub type CompressedGroup = curve25519_dalek::ristretto::CompressedRistretto; // pub struct CompressedRistretto(pub [u8; 32]), CompressedRistretto通过decompress，可以变为RistrettoPoint
...
pub trait CompressedGroupExt {
  type Group;
  fn unpack(&self) -> Result<Self::Group, GroupError>;
}
...
impl CompressedGroupExt for CompressedGroup {
  ...
  fn unpack(&self) -> Result<Self::Group, GroupError> {
    self
      .decompress()
      .ok_or_else(|| GroupError::DecompressionError(self.to_bytes()))
  }
}
...
impl VartimeMultiscalarMul for GroupElement {
  type Scalar = super::super::scalar::Scalar;
  fn vartime_multiscalar_mul<I, J>(scalars: I, points: J) -> Self
  where
    I: IntoIterator,
    I::Item: Borrow<Self::Scalar>,
